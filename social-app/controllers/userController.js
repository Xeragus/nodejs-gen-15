...
const addFriend = async (req, res) => {
  ...
  try {
    ...
    response(
      res,
      201,
      `User with id #${req.user.id} has added to friends user with id #${req.params.id}.`,
      { friendship }
    );
  } catch (error) {
...
const deleteFriend = async (req, res) => {
  try {
    ...
    await Friendship.findOneAndDelete({
      $or: [
        {
          $and: [{ user_one: req.user.id }, { user_two: req.params.id }],
        },
        {
          $and: [{ user_one: req.params.id }, { user_two: req.user.id }],
        },
      ],
    });
    ...
    response(
      res,
      201,
      `User with id #${req.user.id} has deleted the friendship with user with id #${req.params.id}.`
    );
  } catch (error) {
    response(res, 500, error.message, { error });
  }
